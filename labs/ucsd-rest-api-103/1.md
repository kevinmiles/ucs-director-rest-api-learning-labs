...
## Prerequisites
Prior to starting this learning lab it would be helpful to understand UCS Director and the functionality that it provides. Also having completed the UCS Director REST API Learning Lab Part I and Part II would be beneficial.
...
### Exercise 1
Workflows are the key to UCS Director, a workflow allows you to organize tasks both included and custom into a flow to orchestrate and automate a process.  Potentially all the tasks you need are already available in UCS Director, for those tasks that do not exist a custom one can be created.
